...
public class CastleRightPostCondition implements PostConditionValidator {

  private final Validator shortCastleValidator = new ShortCastleValidator();
  ...
  public PostConditionResult validate(GameState gameState, Board updatedBoard) {
    ...
    if (shortCastleValidator.validate(movement, gameState)
  ...
  private Board updateRookPosition(Board board, Movement movement) {
    Position rookPosition = new Position(movement.from().row(), movement.from().column() + 3);
