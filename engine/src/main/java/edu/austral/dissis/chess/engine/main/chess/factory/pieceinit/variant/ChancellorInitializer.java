package edu.austral.dissis.chess.engine.main.chess.factory.pieceinit.variant;
...
public class ChancellorInitializer implements PieceInitializer {
  ...
  public Piece initialize(Color color, String id) {
    ...
    validators.add(
        new OrValidator(
            List.of(new KnightMoveValidator(), Moves.straightMove(), Moves.horizontalMove())));
    validators.add(Moves.destinationPosition());

    Validator finalValidator = new AndValidator(validators);

    return new Piece(id, color, PieceType.CHANCELLOR, finalValidator);
  }
}
